...
- **`forget()` after-commit events now honor SQLite `executescript()` implicit commits (#963).** Connection- and cursor-level scripts drain queued `MEMORY_INVALIDATED` hooks before execution, so a script failure cannot leave an already-committed event delayed until an unrelated later commit.
